...
app.post('/api/image-to-pdf', uploadImages.array('images', 50), async (req, res) => {
  ...
  try {
    ...
    for (const file of sortedFiles) {
      // ⚡ Bolt: Use sequential async readFile instead of synchronous readFileSync
      // This prevents blocking the Node.js event loop during batch processing
      // while avoiding the severe memory spikes that Promise.all() would cause with large files
    ...
    // ⚡ Bolt: Use async writeFile to avoid blocking the event loop when writing large PDFs
