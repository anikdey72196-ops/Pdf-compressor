...
app.post('/api/image-to-pdf', uploadImages.array('images', 50), async (req, res) => {
  ...
  try {
    ...
    for (const file of sortedFiles) {
      // ⚡ Bolt: Replace synchronous fs.readFileSync with async fs.promises.readFile to prevent event loop blocking
    ...
    // ⚡ Bolt: Replace synchronous fs.writeFileSync with async fs.promises.writeFile to prevent event loop blocking
    await fs.promises.writeFile(outputPath, pdfBytes);

    // ⚡ Bolt: Use parallel async deletions for cleanup to prevent event loop blocking during multiple file cleanups
