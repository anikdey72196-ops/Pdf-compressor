...
app.post('/api/image-to-pdf', uploadImages.array('images', 50), async (req, res) => {
  ...
  try {
    ...
    // ⚡ Bolt: Use asynchronous I/O to avoid blocking the event loop when processing multiple images
    ...
    // ⚡ Bolt: Use Promise.all with asynchronous I/O to process cleanup in parallel without blocking the event loop
    await Promise.all(sortedFiles.map(async (file) => {
      try {
        await fs.promises.unlink(file.path);
      } catch (e) {
        if (e.code !== 'ENOENT') console.error('Failed to delete temp image file:', e);
      }
    }));
