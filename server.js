...
app.post('/api/image-to-pdf', uploadImages.array('images', 50), async (req, res) => {
  ...
  try {
    ...
    // ⚡ Bolt: Use sequential async file reads to avoid blocking the event loop and prevent OOM errors
    ...
    // ⚡ Bolt: Use async file write to avoid blocking the event loop when saving large PDFs
    await fs.promises.writeFile(outputPath, pdfBytes);

    // Clean up temporary image files
    // ⚡ Bolt: Process deletions asynchronously and sequentially without race conditions
    for (const file of sortedFiles) {
      try {
